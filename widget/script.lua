...
function migrateMemoryV2(savedata_v1)
    if type(savedata_v1) ~= "table" or savedata_v1.version ~= 1 then
        return savedata_v1
    end

    local spd_unit_exists = {
        ["km/h"] = true,
        ["kph"] = true,
        ["kmph"] = true,
        ["km/hr"] = true,
    ...
    if type(savedata_v1.spd_ui_id) == "number" and savedata_v1.spd_ui_id%1 == 0 then
        savedata_v2.spd_ui_id = savedata_v1.spd_ui_id
    end
    if type(savedata_v1.alt_ui_id) == "number" and savedata_v1.alt_ui_id%1 == 0 then
        savedata_v2.alt_ui_id = savedata_v1.alt_ui_id
    end
    if type(savedata_v1.hostdata) == "table" then
        local hostdata_v1 = savedata_v1.hostdata
        local hostdata_v2 = {
            mode = nil,
            spd_hofs = nil,
            spd_vofs = nil,
        ...
        if type(hostdata_v1.enabled) == "boolean" then
            hostdata_v2.mode = hostdata_v1.enabled and "on" or "off"
        end
        if type(hostdata_v1.spd_hofs) == "number" and -1 <= hostdata_v1.spd_hofs and hostdata_v1.spd_hofs <= 1 then
            hostdata_v2.spd_hofs = hostdata_v1.spd_hofs
        end
        if type(hostdata_v1.spd_vofs) == "number" and -1 <= hostdata_v1.spd_vofs and hostdata_v1.spd_vofs <= 1 then
            hostdata_v2.spd_vofs = hostdata_v1.spd_vofs
        end
        if type(hostdata_v1.spd_unit) == "string" and spd_unit_exists[hostdata_v1.spd_unit] then
            hostdata_v2.spd_unit = hostdata_v1.spd_unit
        end
        if type(hostdata_v1.alt_hofs) == "number" and -1 <= hostdata_v1.alt_hofs and hostdata_v1.alt_hofs <= 1 then
            hostdata_v2.alt_hofs = hostdata_v1.alt_hofs
        end
        if type(hostdata_v1.alt_vofs) == "number" and -1 <= hostdata_v1.alt_vofs and hostdata_v1.alt_vofs <= 1 then
            hostdata_v2.alt_vofs = hostdata_v1.alt_vofs
        end
        if type(hostdata_v1.alt_unit) == "string" and alt_unit_exists[hostdata_v1.alt_unit] then
            hostdata_v2.alt_unit = hostdata_v1.alt_unit
        end

        if next(hostdata_v2) ~= nil then
            savedata_v2.hostdata = hostdata_v2
        end
    end
